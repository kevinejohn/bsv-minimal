...
;(() => {
  ...
  assert.equal(block.transactions.length, 26)
  ...
  // console.log(block)
  assert.equal(block.transactions.length, block2.transactions.length)
  ...
  for (let i = 0; i < block.transactions.length; i++) {
    assert.equal(
      Buffer.compare(blockLite2.txids[i], block.transactions[i].hash),
      0
    )
  }
  ...
  for (let i = 0; i < block.transactions.length; i++) {
    assert.equal(
      Buffer.compare(blockLite3.txids[i], block.transactions[i].hash),
      0
    )
  }
